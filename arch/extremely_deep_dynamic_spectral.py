...
Using n = 4 hypernets. But do spectral normalization every forward pass.
...
class Lowest(BaseNet):
    def create_params(self):
        self.weight_generator = nn.Sequential(
            nn.Flatten(),
            nn.GELU(),
            spectral_norm(nn.Linear(784, 100)),
            nn.GELU(),
            spectral_norm(nn.Linear(100, 10)),
        )
    ...
    def create_params(self):
        class WeightGenerator(nn.Module):
            def __init__(self, num_params_to_estimate, pool_size):
                ...
                # very tiny CNN
                self.conv1 = spectral_norm(nn.Conv2d(1, final_channel_dim, 3, 1, 1))
    ...
    def create_params(self):
        class WeightGenerator(nn.Module):
            def __init__(self, batch_size, num_heads, dropout_rate=0.1):
                super().__init__()
                self.attn = spectral_norm(nn.MultiheadAttention(embed_dim=batch_size, num_heads=num_heads, dropout=dropout_rate))
    ...
    def create_params(self):
        class WeightGenerator(nn.Module):
            def __init__(self, batch_size):
                super().__init__()
                self.linear = spectral_norm(nn.Linear(batch_size, 1))
